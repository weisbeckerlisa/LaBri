...
if nargin >= 1
    ...
    img = double(imread('test_img2.jpg'));
end
...
[c] = c_metric(lab_map);
%[gr] = gr_metric(lab_map); %(please download the 'Global Regularity' metric function at remigiraud.fr/research/gr.php)
...
if (nargin >= 3) %gt is provided
    
    asa = asa_metric(lab_map,gt)
    asa = asa_metric_mex(lab_map,gt);
