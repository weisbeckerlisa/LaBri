...
double compute_cr (int *spx, int spx_size, int n_cols, int n_rows) {
    ...
    double cr = spx_size * hull_perim / (spx_perim * hull_area);
    
    // clean

if (hull_perim > spx_perim) {
         cout << "hull_perim " << hull_perim << " spx_perim " << spx_perim <<"\n";
    }
...
//MAIN
double gr_metric(int *spx[], int *spx_sizes, int n_spx, int n_cols, int n_rows) {
    double src = compute_src(spx, spx_sizes, n_spx, n_cols, n_rows);
    double smf = compute_smf(spx, spx_sizes, n_spx, n_cols, n_rows);
